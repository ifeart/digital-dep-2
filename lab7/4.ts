...
class infoVehicle implements Vehicle {
    ...
    someInfoVehiclePrint(): string {
        let ToJsonStringify = {
            ...
            infoAboutOwner: this.infoAboutOwner
        };
...
class infoVehicleStorage<T extends Vehicle> implements VehicleStorage<T> {
    ...
    getIncludeInfo(rightData: string): T[] {
        return this.vehicle.filter((vehicle) => vehicle.model.slice(-rightData.length) === rightData);
    }
}
...
//Задание 2
console.log('\n\nНовый массив');
console.log(legendCarStorage.getIncludeInfo('X'));

//Задание 3
console.log(timeMachine.someInfoVehiclePrint());