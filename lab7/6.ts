...
class infoMotorBike implements Motorbike {
    ...
    someInfoVehiclePrint(): string {
        let ToJsonStringify = {
            bikeFrame: this.bikeFrame,
            sportBike: this.sportBike
        };
...
class infoVehicleStorage<T extends Vehicle> implements VehicleStorage<T> {
    ...
    sortAllInfo(): void {
        this.vehicle.sort((a, b) => {
            if (a.infoAboutOwner < b.infoAboutOwner) return -1;
            if (a.infoAboutOwner > b.infoAboutOwner) return 1;
            return 0;
        });
    }
    ...
    getIncludeInfo(rightData: any): T[] {
        return this.vehicle.filter((element) => {
            if (element.documentNumber !== undefined) {
                return element.documentNumber.toString().slice(0, rightData.length) === rightData.toString();
            }
...
//Задание 2
console.log('\n\nНовый массив');
console.log(legendCarStorage.getIncludeInfo(123));

//Задание 3
let Arch1S1 = new infoMotorBike(
    'Arch Motorcycle',
    '1S',
    new Date(2022),
    'CYBER77NEO',
...
console.log(Arch1S1.someInfoVehiclePrint());