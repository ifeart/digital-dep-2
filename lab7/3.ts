...
class infoMotorBike implements Motorbike {
    ...
    someInfoVehiclePrint(): void {
        console.log(`\nРама ${this.bikeFrame}\n${this.sportBike ? 'Мотоцикл является спортивным' : 'Мотоцикл не является спортивным'}`);
    }
}
...
class infoVehicleStorage<T extends Vehicle> implements VehicleStorage<T> {
    ...
    getIncludeInfo(rightData: string): T[] {
        return this.vehicle.filter((vehicle) => vehicle.infoAboutOwner.slice(0, rightData.length).toLowerCase() === rightData.toLowerCase());
    }
}
...
//Задание 2
console.log('\n\nНовый массив');
console.log(legendCarStorage.getIncludeInfo('JOHN'));
